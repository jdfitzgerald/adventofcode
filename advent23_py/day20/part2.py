...
for i in range(pushes):
    ...
    while len(next_tick):
        ...
        for (m, src, type) in this_tick:
            if m.name == 'lb':
                print(src, type, m.source_states) # the sources are never updating
                debug_module = modules['lf']
                print(debug_module.source_states) # the sources are never updating
            if m.name == 'rx' and type == LOW_PULSE:
                next_tick=[]
                found = True
                break
            m.pulse(src, type)
